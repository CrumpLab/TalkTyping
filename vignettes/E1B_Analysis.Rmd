...
E1B_data <- E1B_data %>%
            mutate(subject = as.factor(subject),
                   VoiceCondition = as.factor(VoiceCondition),
                   ChunkCondition = as.factor(ChunkCondition),
                   LetterType = as.factor(LetterType)) %>%
            filter(accuracy == 1,
                   iksis < 5000,
                   LetterType != "Space") %>%
            group_by(subject,VoiceCondition,ChunkCondition,LetterType) %>%
...
E1B_aov_out <- aov(mean_iksi ~ VoiceCondition*ChunkCondition*LetterType + 
                    Error(subject/(VoiceCondition*ChunkCondition*LetterType)), E1B_data)
...
E1B_iksi_table <- E1B_data %>%
              group_by(VoiceCondition,ChunkCondition,LetterType) %>%
...
E1B_graph_iksi <- ggplot(E1B_iksi_table, aes(x=LetterType,
                                             y=mIKSI,
                                             group=ChunkCondition,
                                             shape=ChunkCondition,
                                             linetype=ChunkCondition))+
  geom_line()+
...
E1Bacc_data <- E1Bacc_data %>%
                ...
                filter(LetterType != "Space",
                       LetterPosition == 1) %>%
                group_by(subject,VoiceCondition,ChunkCondition) %>%
                summarise(mean_acc = mean(accuracy))
                  
E1Bacc_aov_out <- aov(mean_acc ~ VoiceCondition*ChunkCondition + Error(subject/(VoiceCondition*ChunkCondition)), E1Bacc_data)
...
E1B_acc_table <- E1Bacc_data %>%
              group_by(VoiceCondition,ChunkCondition) %>%
