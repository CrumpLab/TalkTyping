...
Typing speed is fastest for words and decreases as letter strings approach random orders [@shafferTypingPerformanceFunction1968; @behmerCrunchingBigData2017]. This finding is consistent with the idea that typists use word-representations to control fluent typing, and that typing becomes disfluent when word-representations (e.g., for random strings) are unavailable. At the same time, the differences between typing words and non-word strings that approximate the structure of words to various degrees could reflect a frequency sensitive learning process [@loganInstanceTheoryAutomatization1988] that operates at the letter level, or n-gram level (e.g., letter bigrams or trigrams).

First-letter and mid-word slowing phenomenon [@ostryDeterminantsInterkeyTimes1983] in typing show that the first letter in a word is typed more slowly than the remaining letters, and the middle letter in a word is generally typed more slowly than the surrounding letters. First letter slowing could reflect the planning or buffering time necessary for the outer loop to construct and send a word-level represention to the inner loop. Mid-word slowing could reflect a similar planning process operating at the level of syllables [@massaroTypingLetterStrings1984; @pinetTypingWritingLinguistic2016; @willLinguisticUnitsWord2006], which appear in the middle of words. Alternatively, @crumpInstanceTheoryPredictsInpress proposed that first and middle letter slowing could be explained by letter-level informational uncertainty, as a function of letter position, word length, and previous letter contexts. Although the found that letter uncertainty explained a large portion of the variance in keystroke times for letters in the contexts, they also found that first letter keystroke times were longer than would be expected on the basis of letter uncertainty alone. They concluded that first-letter slowing reflects a combination of planning time and learned sensitivity to the predictability of letters in those contexts.

Last, @crumpHierarchicalControlSkilled2010 showed that words and nonwords differentially prime their constituent letters. They presented a word or non-word as a prime, and then had subjects respond to a subsequent letter probe. Word string primes facilitated letters for all positions in a word, compared to letters not in the word. However, non-word string primes only showed facilitation for letter probes from the first position in the string. This finding suggests that a word-level representation plays a role in facilitating the production of all of its constituent letters.
...
```{r E1Figure, echo=FALSE, fig.width=6, fig.height=3, fig.cap="Mean interkeystroke intervals (IKSIs in ms) and mean accuracy as a function of linguistic unit condition (Say Word vs. Say Letter), and letter position in word (first letter vs. middle letter). Middle letters were any remaining letter in the word."}
ggarrange(E1A_graph_iksi,E1A_graph_acc)
```

The main effect of the linguistic unit condition was significant, `r E1_apa_print$full_result$linguistic_unit`. Mean IKSIs were shorter when subjects were instructed to use their inner voice to say the words (`r papaja::printnum(E1_means$tables$linguistic_unit["say_word"], digits=0)`), rather than the letters (`r papaja::printnum(E1_means$tables$linguistic_unit["say_letter"], digits=0)`) they were typing.

The main effect of letter position was significant, `r E1_apa_print$full_result$LetterType`. Mean IKSIs were longer for letters in the first position (`r papaja::printnum(E1_means$tables$LetterType["First"], digits=0)`), compared to letters in the other positions (`r papaja::printnum(E1_means$tables$LetterType["Middle"], digits=0)`).

The interaction between linguistic unit and letter position did not meet the significance criterion, `r E1_apa_print$full_result$linguistic_unit_LetterType`.
...
```{r}
# see E1B_Analysis.Rmd
load("E1B_workspace.RData")
```

The same pre-processing rules were applied to the data. The outlier elimination procedure excluded an average of  `r round(mean(E1B_data$prop_removed), digits=2)` observations from each condition.  Mean IKSIs and accuracies for each condition are displayed in Figure \@ref(fig:E1BFigure)

```{r E1BFigure, fig.cap="Mean IKSIs in milliseconds and mean accuracy for typing the first letter in a word, or all other letters (middle letters) for each of the five paragraph typing conditions. For each letter type, the scatterplot shows mean accuracy by mean IKSI for each typing condition.",fig.width=6.875, fig.env="figure*", fig.height=3.25}
...
```{r E1BFL, fig.cap="Mean IKSIs in milliseconds and mean accuracy for typing the first letter in a word, or all other letters (middle letters) for each of the five paragraph typing conditions. The scatterplots for each letter type show mean accuracy by mean IKSI for each typing condition.",fig.width=6.875, fig.env="figure*", fig.height=3.25, eval=FALSE}
...
The main effect of paragraph typing condition was significant, `r E1B_FL_apa_print$full_result$condition`. Linear contrasts were constructed to interpret the major patterns. First, normal typing compared to silent-word and talk-word conditions were not different, `r E1B_FL_NvsW$full_result`. The instruction to say each word (silently or out loud) while typing did not facilitate or interfere with first letter typing time compared to normal typing. One possibility is that normal typing typically involves saying words while typing, so the added instruction may be redundant with normal activity during typing. 

Second, mean first letter IKSIs for silent-letter and talk-letter conditions were shorter than the other conditions, `r E1B_FL_NWvsL$full_result`. The data clearly show that first-letter typing time increases when subjects say letters rather than words while typing. Finally, saying letters out loud appeared to slow down first-letter typing compared to saying letters silently, `r E1B_FL_SLvsTL$full_result`.

#### Middle letter IKSIs

A corresponding ANOVA was conducted on IKSIs for all correctly typed middle letters. 

The main effect of paragraph typing condition was significant, `r E1B_ML_apa_print$full_result$condition`. Mean middle letter IKSIs during normal typing were longer compared to silent-word and talk-word conditions, `r E1B_ML_NvsW$full_result`. Saying words silently or out loud appeared to facilitate typing speed by a small, yet detectable amount.

Mean IKSIs for silent-letter and talk-letter conditions were much longer than the other conditions, `r E1B_ML_NWvsL$full_result`. But, there was no difference between the silent-letter and talk-letter conditions, `r E1B_ML_SLvsTL$full_result`.

#### Accuracy

The analysis of accuracy showed a main effect of linguistic unit, `r E1Bacc_apa_print$full_result$linguistic_unit`. Subjects were more accurate when saying letters (`r papaja::printnum(E1Bacc_means$tables$linguistic_unit["Letter"], digits=2)`), than words (`r papaja::printnum(E1Bacc_means$tables$linguistic_unit["Word"], digits=2)` ms). The main effect of letter position was significant, `r E1Bacc_apa_print$full_result$LetterType`. Subjects were more accurate for the first letter (`r papaja::printnum(E1Bacc_means$tables$LetterType["First"], digits=2)`), compared to other letters  (`r papaja::printnum(E1Bacc_means$tables$LetterType["Middle"], digits=2)`). The main effect of voice was not significant, `r E1Bacc_apa_print$full_result$voice`. These main effects were qualified by two-way interactions.

The interaction between voice and linguistic unit was significant, `r E1Bacc_apa_print$full_result$voice_linguistic_unit`. The decrease in accuracy for saying words vs letters was larger when subjects spoke out loud, compared to speaking silently with their inner voice. The interaction between voice and letter position was significant, `r E1Bacc_apa_print$full_result$voice_LetterType`. The decrease in an accuracy for middle letters vs first letters was larger when subjects spoke out loud, compared to speaking silently. The interaction between linguistic unit and letter position was significant, `r E1Bacc_apa_print$full_result$linguistic_unit_LetterType`. The decrease in accuracy for middle letters vs first letters was larger when subjects said the word rather than the letter they were typing. The three-way interaction was not significant, `r E1Bacc_apa_print$full_result$voice_linguistic_unit_LetterType`
...
load("E2_workspace.RData")
```

The same pre-processing rules were applied to the data. The outlier elimination procedure excluded an average of  `r round(mean(E2_data$prop_removed), digits=2)` observations from each condition.  Mean IKSIs and accuracies for each condition are displayed in Figure \@ref(fig:E2).


```{r E2, fig.cap="Mean IKSIs in milliseconds and mean accuracy for typing the first letter in a word, or all other letters (middle letters) for each of the six paragraph typing conditions. Note, the range of mean IKSIs for the middle letters is truncated to show differences between conditions. For each letter type, the scatterplot shows mean accuracy by mean IKSI for each typing condition.",fig.width=6.875, fig.env="figure*", fig.height=3.25, eval=TRUE}

grid.arrange(
  ggarrange(E2_FL_graph_iksi,E2_ML_graph_iksi, 
            E2acc_FL_graph_acc, E2acc_ML_graph_acc, nrow=2, ncol=2,
            labels = list(A="First Letter ",
                          B="Middle Letters"),
            font.label = list(size=10),
            hjust=-.8),
...
Mean correctly typed first letter IKSIs for each subject were submitted to a one-way repeated measures ANOVA. The main effect of paragraph typing condition was significant, `r E2_FL_apa_print$full_result$suppression`. Linear contrasts were constructed to interpret the major patterns. First letter mean IKSIs during normal typing were not significantly different from the Say The, and Tuesday Thursday conditions, `r E2_FL_NvsTheThu$full_result`. First letter mean IKSIs were longer when saying the Alphabet compared to Say The, and Tuesday Thursday, `r E2_FL_AlphavsTheThu$full_result`. Finally, first letter mean IKSIs were longest while saying Random Letters and Counting compared to the Alphabet condition, `r E2_FL_AlphavsRandCnt$full_result`.

### Middle letter IKSIs

Mean correctly typed middle letter IKSIs for each subject were submitted to a one-way repeated measures ANOVA. The main effect of paragraph typing condition was significant, `r E2_ML_apa_print$full_result$suppression`. Middle letter mean IKSIs during normal typing were not significantly different from the Say The, Tuesday Thursday, Alphabet, and Counting conditions, `r E2_ML_NvsAllbR$full_result`. Middle letter mean IKSIs were longer in the Random Letter condition compared to normal typing, `r E2_ML_RandvsNormal$full_result`.

### Accuracy

Mean first letter accuracies for each subject in each condition were submitted to a one-way repeated measures ANOVA. The main effect of paragraph typing condition was significant, `r E2acc_FL_apa_print$full_result$suppression`. Mean accuracy during normal typing was not different from the Say The and Tuesday Thursday conditions, `r E2acc_FL_NvsTheThur$full_result`. Mean accuracy for the Alphabet, Random Letter, and Counting conditions was significantly worse compared to the former conditions, `r E2acc_FL_NTheThvsARC$full_result`.

For middle letter accuracies, the main effect of paragraph typing condition was significant, `r E2acc_ML_apa_print$full_result$suppression`. Mean accuracy during normal typing was not different from the Say The and Tuesday Thursday conditions, `r E2acc_ML_NvsTheThur$full_result`. Mean accuracy for the Alphabet, Random Letter, and Counting conditions was significantly worse compared to the former conditions, `r E2acc_ML_NTheThvsARC$full_result`.
...
The same apparatus and stimuli from Experiment 1b and 2 were used in Experiment 3.

### Design and Procedure. 

The same general typing test procedure from Experiment 1b was used, with the exception that typists copied X paragraphs. The instructions for each of the conditions were as follows:

## Results

```{r, echo=FALSE, eval=TRUE}
load("E3_workspace.RData")
```

The same pre-processing rules were applied to the data. The outlier elimination procedure excluded an average of  `r round(mean(E3_data$prop_removed), digits=2)` observations from each condition.  Mean IKSIs and accuracies for each condition are displayed in Figure \@ref(fig:E3).

```{r E3, fig.cap="Mean IKSIs in milliseconds and mean accuracy for typing the first letter in a word, or all other letters (middle letters) for each of the delayedd auditry feedback conditins. For each letter type, the scatterplot shows mean accuracy by mean IKSI for each typing condition.",fig.width=6.875, fig.env="figure*", fig.height=3.25, eval=TRUE}

grid.arrange(
  ggarrange(E3_FL_graph_iksi,E3_ML_graph_iksi, 
            E3acc_FL_graph_acc, E3acc_ML_graph_acc, nrow=2, ncol=2,
            labels = list(A="First Letter ",
                          B="Middle Letters"),
            font.label = list(size=10),
            hjust=-.8),
...
Mean correctly typed first letter IKSIs for each subject were submitted to a one-way repeated measures ANOVA. The main effect of paragraph typing condition was not significant, `r E3_FL_apa_print$full_result$delay`. Similarly, for the companion ANOVA for the middle letter IKSIs, the main effect of paragraph typing condition was not significant, `r E3_ML_apa_print$full_result$delay`.

### Accuracy

Mean first letter accuracies for each subject in each condition were submitted to a one-way repeated measures ANOVA. The main effect of paragraph typing condition was significant, `r E3acc_FL_apa_print$full_result$delay`. Mean accuracy was higher for the 100 ms delay condition compared to the 0 ms delay condition, `r E3acc_FL_0vs100$full_result`, and to the 200 ms delay condition, `r E3acc_FL_200vs100$full_result`. Mean accuracy was not different for the 0 ms and 200 ms conditions, `r E3acc_FL_0vs200$full_result`.

For middle letter accuracies, the main effect of paragraph typing condition was significant, `r E3acc_ML_apa_print$full_result$delay`. Mean accuracy was not different for the 0 ms and 100 ms conditions, `r E3acc_ML_0vs100$full_result`. Mean accuracy was lower for the 200 ms delay condition compared to the 0 ms delay condition, `r E3acc_ML_0vs200$full_result`, and to the 100 ms delay condition, `r E3acc_ML_200vs100$full_result`.
