...
E1_data <- E1_data %>%
            ...
            filter(errors == 1,
                   iksis < 5000,
                   LetterType != "Space") %>%
            group_by(subject,paragraph,LetterType) %>%
...
E1_aov_out <- aov(mean_iksi ~ paragraph*LetterType + 
                    Error(subject/(paragraph*LetterType)), E1_data)
...
E1A_graph_iksi <- ggplot(E1A_iksi_table, aes(x=LetterType,y=mIKSI, group=paragraph,
                         shape=paragraph,
                      linetype=paragraph))+
  geom_line()+
...
E1acc_aov_out <- aov(mean_acc ~ paragraph*LetterType + Error(subject/(paragraph*LetterType)), E1acc_data)
