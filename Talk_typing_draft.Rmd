...
author_note: |
  [https://github.com/CrumpLab/TalkTyping](https://github.com/CrumpLab/TalkTyping) is the github repository for this project, which was conceived and completed entirely in public. The repository contains the data, source code for compiling the analysis and modeling scripts in R, source code for compiling this paper in R using papaja, and the version controlled history of our discussions and work across the project.
...
Mean IKSIs for correctly typed letters were computed for each subject in each condition and submitted to a one-way repeated measures ANOVA with inner voice task condition (words vs. letters) as the sole factor. Mean IKSIs in the word condition (174 ms) were significantly faster than the letter condition (224 ms), F(1, 39)  = 33.39 , MSE = 1474.36 , p < .001. A corresponding analysis of error rates showed no significant differences between word (.04) and letter (.05) conditions, F<1.
